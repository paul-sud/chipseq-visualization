...
def sequential_encsr_encff(args):
    ...
    for encsrName, encsrLink in zip(encsrNameList, encsrLinkList):
        ...
        for element in range(len(infoFiltered)):
            if (
                infoFiltered[element]["file_type"] == "bed narrowPeak"
                and infoFiltered[element]["output_type"] == "replicated peaks"
            ):
    ...
    if fileType == "bed":
        for element in range(len(info_filtered)):
            for assemblyElement, outputTypeElement in zip(assembly, outputType):
                if (
                    info_filtered[element]["file_type"] == "bed narrowPeak"
                    and info_filtered[element]["output_type"] == outputTypeElement
                ):
                    if info_filtered[element]["assembly"] == assemblyElement:
                        ...
                        encff_label = (
                            info_filtered[element]["s3_uri"][-18:]
                            + " "
                            + info_more["biosample_ontology"]["term_name"]
                            + " "
                            + info_more["target"]["label"]
                        )
    ...
    if fileType == ["bigBed"]:
        with open("input.txt", "a+") as f:
            for element in range(len(info_filtered)):
                for assemblyElement, outputTypeElement in zip(assembly, outputType):
                    if (
                        info_filtered[element]["file_type"] == "bigBed narrowPeak"
                        and info_filtered[element]["output_type"] == outputTypeElement
                    ):
                        if info_filtered[element]["assembly"] == assemblyElement:
                            encff_names.append(info_filtered[element]["s3_uri"][-18:])
                            f.write(
                                info_filtered[element]["cloud_metadata"]["url"] + "\n"
                            )
